package com.bookcollector.task.repository;
...
public class TaskRepository {
     ...
     * 本类只负责「条件怎么拼」（{@link #buildRunCriteria}）和「按什么排」。
     */
    public PageResult<TaskRun> pageRuns(String taskId, String status, int page, int size) {
        // 主排序 startedAt 倒序（新的在前）+ _id 升序兜底，保证翻页不重不漏
        ...
        return PageQueryUtil.getPageResult(mongoTemplate, page, size,
                buildRunCriteria(taskId, status), TaskRun.class, sort);
    }

    /** 多个条件用 andOperator 组合，避免同字段叠加时被覆盖 */
    private Criteria buildRunCriteria(String taskId, String status) {

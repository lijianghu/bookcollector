 ...
 * <h3>🔴 状态写入的分工（重要）</h3>
 * <ul>
 *   <li><b>本类</b>写中间态（RUNNING / PAUSED / CANCELED），一律走
 *       {@link TaskRepository#casTaskStatus}（乐观锁）—— 因为 HTTP 线程可能并发操作同一任务。</li>
 *   <li><b>{@link TaskRunner}</b> 写终态，无条件覆盖 —— 它是这次运行的所有者，
 *       终态是权威结果，必须能盖掉运行期间用户点的暂停。</li>
 * </ul>
 * 这条分工不能乱：如果本类也写终态，会和 TaskRunner 抢；
...
public class TaskService {
    ...
    public PageResult<TaskRun> pageRuns(String taskId, String status, Integer page, Integer size) {
        ...
        long total = repository.countRuns(taskId, status);
        if (total == 0L) {
            return PageResult.empty(p, s);
        }
        List<TaskRun> list = repository.findRunsPage(taskId, status, p, s);
